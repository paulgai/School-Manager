...
<script>
	$j(function() {
		...
		var data = {
			...
			projector_id: <?php echo json_encode(['id' => $rdata['projector_id'], 'value' => $rdata['projector_id'], 'text' => $jdata['projector_id']]); ?>
		};
		...
		/* saved value for projector_id */
		cache.addCheck(function(u, d) {
			if(u != 'ajax_combo.php') return false;
			if(d.t == tn && d.f == 'projector_id' && d.id == data.projector_id.id)
				return { results: [ data.projector_id ], more: false, elapsed: 0.01 };
			return false;
		});
