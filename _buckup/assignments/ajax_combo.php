	...
	$lookups = [
		'Assignments' => [
			...
			'TeacherID' => [
				...
				'parent_from' => '`Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` ',
				'filterers' => [],
				'custom_query' => '',
				'inherit_permissions' => false,
				'list_type' => 0,
		...
		'Projectors_timetable' => [
			...
			'hour_id' => [
				'parent_table' => 'Hours',
				'parent_pk_field' => 'id',
				'parent_caption' => 'IF(CHAR_LENGTH(`Hours`.`name`) || CHAR_LENGTH(`Hours`.`start_time`), CONCAT_WS(\'\', `Hours`.`name`, \' \', `Hours`.`start_time`), \'\')',
				'parent_from' => '`Hours` ',
				'filterers' => [],
				'custom_query' => 'SELECT `Hours`.`id`, IF(CHAR_LENGTH(`Hours`.`name`) || CHAR_LENGTH(`Hours`.`start_time`), CONCAT_WS(\'\', `Hours`.`name`, \' \', `Hours`.`start_time`), \'\') FROM `Hours` ORDER BY 2',
				'inherit_permissions' => false,
				'list_type' => 0,
				'not_null' => true,
			],
