<?php
	...
	*/
	...
	function get_table_groups($skip_authentication = false) {
		...
		$all_groups = ['None', '&#913;&#957;&#945;&#952;&#941;&#963;&#949;&#953;&#962;', '&#933;&#960;&#949;&#973;&#952;&#965;&#957;&#959;&#953; &#932;&#956;&#951;&#956;&#940;&#964;&#969;&#957;', '&#914;&#953;&#957;&#964;&#949;&#959;&#960;&#961;&#959;&#946;&#959;&#955;&#949;&#943;&#962;'];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			...
			'Teachers' => "`Teachers`.`id` as 'id', IF(    CHAR_LENGTH(`Sectors1`.`Sector`), CONCAT_WS('',   `Sectors1`.`Sector`), '') as 'SectorID', `Teachers`.`Name_Sector` as 'Name_Sector', `Teachers`.`Name` as 'Name', `Teachers`.`Placement` as 'Placement', DATE_FORMAT(`Teachers`.`Assumption_Date`, '%d/%m/%Y') as 'Assumption_Date', `Teachers`.`Mandatory_Hours` as 'Mandatory_Hours', `Teachers`.`Main_Sector` as 'Main_Sector', `Teachers`.`Assigned_Hours_Theory` as 'Assigned_Hours_Theory', `Teachers`.`Assigned_Hours_Lab` as 'Assigned_Hours_Lab', `Teachers`.`Assigned_Hours` as 'Assigned_Hours', `Teachers`.`Diff` as 'Diff'",
			'Lessons' => "`Lessons`.`id` as 'id', `Lessons`.`Class` as 'Class', `Lessons`.`Name` as 'Name', `Lessons`.`Type` as 'Type', `Lessons`.`Hours` as 'Hours', `Lessons`.`NameTypeHours` as 'NameTypeHours', `Lessons`.`A_assignment` as 'A_assignment', `Lessons`.`B_assignment` as 'B_assignment', `Lessons`.`C_assignment` as 'C_assignment'",
			...
			'Heads' => "`Heads`.`id` as 'id', IF(    CHAR_LENGTH(`Teachers1`.`Name_Sector`), CONCAT_WS('',   `Teachers1`.`Name_Sector`), '') as 'teacher_id', IF(    CHAR_LENGTH(`Classes1`.`Class`), CONCAT_WS('',   `Classes1`.`Class`), '') as 'class_id'",
			'Projectors_timetable' => "`Projectors_timetable`.`id` as 'id', IF(    CHAR_LENGTH(`Teachers1`.`Name_Sector`), CONCAT_WS('',   `Teachers1`.`Name_Sector`), '') as 'teacher_id', if(`Projectors_timetable`.`date`,date_format(`Projectors_timetable`.`date`,'%d/%m/%Y'),'') as 'date', IF(    CHAR_LENGTH(`Hours1`.`name`) || CHAR_LENGTH(`Hours1`.`start_time`), CONCAT_WS('',   `Hours1`.`name`, ' ', `Hours1`.`start_time`), '') as 'hour_id', IF(    CHAR_LENGTH(`Projectors1`.`name`), CONCAT_WS('',   `Projectors1`.`name`), '') as 'projector_id', `Projectors_timetable`.`star_time` as 'star_time', `Projectors_timetable`.`end_time` as 'end_time'",
			'Hours' => "`Hours`.`id` as 'id', `Hours`.`name` as 'name', `Hours`.`start_time` as 'start_time', `Hours`.`end_time` as 'end_time'",
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			...
			'Teachers' => "`Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` ",
			'Lessons' => "`Lessons` ",
			'Classes' => "`Classes` ",
			'Sectors' => "`Sectors` ",
			'Heads' => "`Heads` LEFT JOIN `Teachers` as Teachers1 ON `Teachers1`.`id`=`Heads`.`teacher_id` LEFT JOIN `Classes` as Classes1 ON `Classes1`.`id`=`Heads`.`class_id` ",
			'Projectors_timetable' => "`Projectors_timetable` LEFT JOIN `Teachers` as Teachers1 ON `Teachers1`.`id`=`Projectors_timetable`.`teacher_id` LEFT JOIN `Hours` as Hours1 ON `Hours1`.`id`=`Projectors_timetable`.`hour_id` LEFT JOIN `Projectors` as Projectors1 ON `Projectors1`.`id`=`Projectors_timetable`.`projector_id` ",
			'Hours' => "`Hours` ",
		...
		$pcConfig = [
			...
			'Teachers' => [
				'SectorID' => [
					...
					'display-add-new' => true,
					'forced-where' => '',
					'display-fields' => [1 => '&#932;&#959;&#956;&#941;&#945;&#962;', 2 => '&#917;&#954;&#960;&#945;&#953;&#948;&#949;&#965;&#964;&#953;&#954;&#972;&#962;', 4 => '&#932;&#959;&#960;&#959;&#952;&#941;&#964;&#951;&#963;&#951;', 5 => '&#913;&#957;&#940;&#955;&#951;&#968;&#951; &#933;&#960;&#951;&#961;&#949;&#963;&#943;&#945;&#962;', 6 => '&#933;&#960;&#959;&#967;&#961;&#949;&#969;&#964;&#953;&#954;&#941;&#962;', 8 => '&#920;&#949;&#969;&#961;&#943;&#949;&#962;', 9 => '&#917;&#961;&#947;&#945;&#963;&#964;&#942;&#961;&#953;&#945;', 10 => '&#931;&#973;&#957;&#959;&#955;&#959;', 11 => '&#916;&#953;&#945;&#966;&#959;&#961;&#940;'],
					'display-field-names' => [1 => 'SectorID', 2 => 'Name_Sector', 4 => 'Placement', 5 => 'Assumption_Date', 6 => 'Mandatory_Hours', 8 => 'Assigned_Hours_Theory', 9 => 'Assigned_Hours_Lab', 10 => 'Assigned_Hours', 11 => 'Diff'],
					'sortable-fields' => [0 => '`Teachers`.`id`', 1 => '`Sectors1`.`Sector`', 2 => 3, 3 => 4, 4 => 5, 5 => '`Teachers`.`Assumption_Date`', 6 => '`Teachers`.`Mandatory_Hours`', 7 => 8, 8 => '`Teachers`.`Assigned_Hours_Theory`', 9 => '`Teachers`.`Assigned_Hours_Lab`', 10 => '`Teachers`.`Assigned_Hours`', 11 => '`Teachers`.`Diff`'],
					'records-per-page' => 10,
					'default-sort-by' => 3,
					'default-sort-direction' => 'asc',
					'open-detail-view-on-click' => true,
					...
					'query' => "SELECT `Teachers`.`id` as 'id', IF(    CHAR_LENGTH(`Sectors1`.`Sector`), CONCAT_WS('',   `Sectors1`.`Sector`), '') as 'SectorID', `Teachers`.`Name_Sector` as 'Name_Sector', `Teachers`.`Name` as 'Name', `Teachers`.`Placement` as 'Placement', DATE_FORMAT(`Teachers`.`Assumption_Date`, '%d/%m/%Y') as 'Assumption_Date', `Teachers`.`Mandatory_Hours` as 'Mandatory_Hours', `Teachers`.`Main_Sector` as 'Main_Sector', `Teachers`.`Assigned_Hours_Theory` as 'Assigned_Hours_Theory', `Teachers`.`Assigned_Hours_Lab` as 'Assigned_Hours_Lab', `Teachers`.`Assigned_Hours` as 'Assigned_Hours', `Teachers`.`Diff` as 'Diff' FROM `Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` "
				],
			],
			'Lessons' => [
			],
