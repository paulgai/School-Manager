...
function getTableList($skip_authentication = false)
{
	...
	$arrTables = [
		...
		'Projectors_timetable' => ['&#935;&#961;&#959;&#957;&#959;&#955;&#972;&#947;&#953;&#959; &#914;&#953;&#957;&#964;&#949;&#959;&#960;&#961;&#959;&#946;&#959;&#955;&#941;&#969;&#957; ', '', 'resources/table_icons/projector.png', '&#914;&#953;&#957;&#964;&#949;&#959;&#960;&#961;&#959;&#946;&#959;&#955;&#949;&#943;&#962;'],
		'Hours' => ['&#911;&#961;&#949;&#962;', '', 'resources/table_icons/clock_.png', '&#914;&#953;&#957;&#964;&#949;&#959;&#960;&#961;&#959;&#946;&#959;&#955;&#949;&#943;&#962;'],
		'Projectors' => ['&#914;&#953;&#957;&#964;&#949;&#959;&#960;&#961;&#959;&#946;&#959;&#955;&#949;&#943;&#962;', '', 'resources/table_icons/timetable.png', '&#914;&#953;&#957;&#964;&#949;&#959;&#960;&#961;&#959;&#946;&#959;&#955;&#949;&#943;&#962;'],
	];
...
function configure_anonymous_group()
{
	...
	sql(
		"INSERT INTO `membership_groups` SET 
			...
			$j(function() {
				...
				window.show_notification = function(options) {
					...
					if (options.dismiss_seconds > 0) {
						setTimeout(function() {
							this_notif.addClass('invisible');
						}, options.dismiss_seconds * 1000);
					}
	...
	return [
		...
		'Teachers' => [
			...
			'Assigned_Hours_Theory' => 'SELECT IFNULL((SELECT SUM(`Assignments`.`LessonHours`)
					...
					AND `Assignments`.`LessonType`=N\'ΘΕΩΡΙΑ\'), 0)',
			'Assigned_Hours_Lab' => 'SELECT IFNULL((SELECT SUM(`Assignments`.`LessonHours`)
					FROM `Assignments`
					WHERE (`Assignments`.`TeacherID`=%ID% OR `Assignments`.`TeacherID2`=%ID%)
					AND `Assignments`.`LessonType`=N\'ΕΡΓΑΣΤΗΡΙΟ\'),0)',
		...
		'Lessons' => [
			'NameTypeHours' => 'SELECT CONCAT_WS( \'\', `Lessons`.`Class`, \' - \',`Lessons`.`Name`, \' (\', `Lessons`.`Type`, \') \', `Lessons`.`Hours`, N\' ΩΡΕΣ\') 
...
function checkAppRequirements()
{
	...
	exit('<div style="padding: 3em; font-size: 1.5em; color: #A94442; line-height: 150%; font-family: arial; text-rendering: optimizelegibility; text-shadow: 0px 0px 1px;">' .
		'<ul><li>' .
...
function lookupQuery($tn, $lookupField)
{
	...
	$lookupQuery = [
		'Assignments' => [
			...
			'TeacherID' => 'SELECT `Teachers`.`id`, `Teachers`.`Name_Sector` FROM `Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` ORDER BY 2',
			'TeacherID2' => 'SELECT `Teachers`.`id`, `Teachers`.`Name_Sector` FROM `Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` ORDER BY 2',
		],
		'Teachers' => [
		...
		'Sectors' => [],
		'Heads' => [
			'teacher_id' => 'SELECT `Teachers`.`id`, `Teachers`.`Name_Sector` FROM `Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` ORDER BY 2',
			'class_id' => 'SELECT `Classes`.`id`, `Classes`.`Class` FROM `Classes` ORDER BY 2',
		],
		'Projectors_timetable' => [
			'teacher_id' => 'SELECT `Teachers`.`id`, `Teachers`.`Name_Sector` FROM `Teachers` LEFT JOIN `Sectors` as Sectors1 ON `Sectors1`.`id`=`Teachers`.`SectorID` ORDER BY 2',
			'hour_id' => 'SELECT `Hours`.`id`, IF(CHAR_LENGTH(`Hours`.`name`) || CHAR_LENGTH(`Hours`.`start_time`), CONCAT_WS(\'\', `Hours`.`name`, \' \', `Hours`.`start_time`), \'\') FROM `Hours` ORDER BY 2',
