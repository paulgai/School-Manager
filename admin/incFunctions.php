		...
		return [
			...
			'Teachers' => [
				...
				'Assigned_Hours_Theory' => 'SELECT IFNULL((SELECT SUM(`Assignments`.`LessonHours`)
					...
					AND `Assignments`.`LessonType`=N\'ÈÅÙÑÉÁ\'), 0)',
				'Assigned_Hours_Lab' => 'SELECT IFNULL((SELECT SUM(`Assignments`.`LessonHours`)
					FROM `Assignments`
					WHERE (`Assignments`.`TeacherID`=%ID% OR `Assignments`.`TeacherID2`=%ID%)
					AND `Assignments`.`LessonType`=N\'ÅÑÃÁÓÔÇÑÉÏ\'),0)',
			...
			'Lessons' => [
				'NameTypeHours' => 'SELECT CONCAT_WS( \'\', `Lessons`.`Class`, \' - \',`Lessons`.`Name`, \' (\', `Lessons`.`Type`, \') \', `Lessons`.`Hours`, N\' ÙÑÅÓ\') 
